...
func runArgs(args []string, loopFunc func() error) (gerr error) {
	defer func() {
		...
		case ErrExit:
			slog.Info("runArgs: recovered ErrExit", "code", int(r))
			if r != 0 {
				gerr = r
			}
		default:
			slog.Error("runArgs: recovered panic", "err", r, "stack", string(debug.Stack()))
			panic(r)
		case nil:
			// ok
		}
	...
	} else if noxClient.Seat != nil && noxClient.Seat.ScreenSize().X > 0 {
		noxClient.videoSetGameMode(noxClient.Seat.ScreenSize())
	} else {
