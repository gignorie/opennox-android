...
func (s *Server) nox_server_loadMapFile_4CF5F0(mname string, noCrypt bool) error {
	slog.Info("loadMapFile: entered", "mname", mname, "noCrypt", noCrypt)
	gameLog.Printf("loading map %q", mname)
	cntGameMap.WithLabelValues(filepath.Base(mname)).Inc()
	slog.Info("loadMapFile: Sub_481410")
	legacy.Sub_481410()
	slog.Info("loadMapFile: ObjectsAddPending")
	s.ObjectsAddPending()
	slog.Info("loadMapFile: Nox_xxx_waypoint_5799C0")
	s.Nox_xxx_waypoint_5799C0()
	if mname == "" {
		slog.Error("loadMapFile: empty map name")
	...
	slog.Info("loadMapFile: resolving map file", "fname", fname)
	if _, err := ifs.Stat(fname); err != nil {
		slog.Warn("loadMapFile: stat fname failed, trying nxz", "err", err)
		zname := strings.TrimSuffix(fname, filepath.Ext(mname)) + ".nxz"
		if _, err := ifs.Stat(zname); err != nil {
			slog.Error("loadMapFile: stat zname failed", "err", err)
			return err
		}
	...
	slog.Info("loadMapFile: map file exists on ifs")
	v8 := s.getServerMap()
