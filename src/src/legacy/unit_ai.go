...
func nox_xxx_generateRetreatPath_50CA00(ptr unsafe.Pointer, sz int, obj *nox_object_t, p *C.float2) int {
	pt := readUnalignedPointfPtr(unsafe.Pointer(p))
	buf := make([]types.Pointf, sz)
	n := GetServer().Nox_xxx_generateRetreatPath_50CA00(buf, asObjectS(obj), pt)
	if ptr != nil && n > 0 {
		for i := 0; i < n && i < sz; i++ {
			writeUnalignedPointf(unsafe.Add(ptr, i*8), buf[i])
		}
	}
	return n
}
...
func nox_xxx_creatureSetDetailedPath_50D220(obj *nox_object_t, p *C.float2) {
	pt := readUnalignedPointfPtr(unsafe.Pointer(p))
	GetServer().Nox_xxx_creatureSetDetailedPath_50D220(asObjectS(obj), pt)
}

//export sub_50B810
func sub_50B810(obj *nox_object_t, p *C.float2) int {
	pt := readUnalignedPointfPtr(unsafe.Pointer(p))
	return bool2int(GetServer().Sub_50B810(asObjectS(obj), pt))
}

//export sub_50CB20
func sub_50CB20(obj *nox_object_t, p *C.float2) *nox_waypoint_t {
	pt := readUnalignedPointfPtr(unsafe.Pointer(p))
	return asWaypointC(GetServer().Sub_50CB20(asObjectS(obj), pt))
}
