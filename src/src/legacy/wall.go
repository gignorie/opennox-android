...
func nox_xxx_wallFindCloseSound_410F20(ind int) *C.char {
	...
	return internCStr(d.CloseSound())
}
...
func nox_xxx_mapDamageToWalls_534FC0(a1 *C.int4, a2 unsafe.Pointer, a3 C.float, a4, a5 int, a6 unsafe.Pointer) C.int {
	rect := image.Rect(int(a1.field_0), int(a1.field_4), int(a1.field_8), int(a1.field_C))
	var pt types.Pointf
	if a2 != nil {
		for i := 0; i < 8; i++ {
			*(*byte)(unsafe.Add(unsafe.Pointer(&pt), i)) = *(*byte)(unsafe.Add(a2, i))
		}
	}
	if GetServer().Nox_xxx_mapDamageToWalls_534FC0(rect, pt, float32(a3), a4, object.DamageType(a5), AsObjectP(a6)) {
		return 1
	}
	return 0
}
