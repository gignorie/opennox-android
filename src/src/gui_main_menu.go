...
func winMainMenuAnimOutDoneFnc() int {
	slog.Info("winMainMenuAnimOutDoneFnc: animation out completed")
	...
	winMainMenu.Destroy()
	winMainMenu = nil
	slog.Info("winMainMenuAnimOutDoneFnc: calling ani.Func13", "ptr", ani.Func13Ptr)
	res := ani.Func13()
	slog.Info("winMainMenuAnimOutDoneFnc: ani.Func13 returned", "res", res)
	return 1
}
...
func nox_xxx_windowMainMenuProc_4A1DC0(a1 *gui.Window, ev gui.WindowEvent) gui.WindowEventResp {
	...
	case *WindowEvent0x4007:
		...
		case 111: // Solo campaign button
			slog.Info("GUI: Solo Campaign clicked")
			noxServer.Announce = false
			hasSolo := nox_xxx_checkHasSoloMaps()
			slog.Info("GUI: checkHasSoloMaps", "hasSolo", hasSolo)
			if hasSolo {
				...
				slog.Info("GUI: calling parseGamedataBinPre")
				parseRes := nox_xxx_parseGamedataBinPre_4D1630()
				slog.Info("GUI: parseGamedataBinPre returned", "res", parseRes)
				if parseRes == 0 {
					slog.Error("GUI: parseGamedataBinPre failed, aborting solo")
					nox_xxx_setContinueMenuOrHost_43DDD0(0)
					nox_client_gui_flag_815132 = 0
					return nil
				}
				saveCount := legacy.Nox_client_countSaveFiles_4DC550()
				slog.Info("GUI: countSaveFiles", "count", saveCount)
				if saveCount != 0 {
					legacy.Sub_4A7A70(1)
				...
				slog.Info("GUI: set Func13Ptr", "ptr", winMainMenuAnimTop.Func13Ptr)
				clientPlaySoundSpecial(sound.SoundShellClick, 100)
			} else {
				slog.Warn("GUI: checkHasSoloMaps was false!")
