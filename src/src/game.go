	...
	noxCmdShow.Register(&console.Command{
		...
		Func: func(ctx context.Context, c *console.Console, tokens []string) bool {
			...
			if noxflags.HasGame(noxflags.GameOnline) {
				...
				c.Print(console.ColorRed, s.GetExtIP(s.NetStr.Host()).String())
			} else {
				maxPlayers := s.getServerMaxPlayers()
				mname := s.nox_server_currentMapGetFilename_409B30()
				format := c.Strings().GetStringInFile("GameInfo", "parsecmd.c")
...
func nox_xxx_parseGamedataBinPre_4D1630() int {
	s := noxServer
	slog.Info("parseGamedata: reading balance...")
	if err := s.Balance.Read(); err != nil {
		slog.Error("parseGamedata: s.Balance.Read failed", "err", err)
		return 0
	}
	slog.Info("parseGamedata: reloadGamedata...")
	s.abilities.reloadGamedata()
	slog.Info("parseGamedata: CalcClassStats...")
	s.CalcClassStats()
	slog.Info("parseGamedata: loadMonsterBin...")
	mres := legacy.Nox_xxx_loadMonsterBin_517010()
	slog.Info("parseGamedata: loadMonsterBin done", "res", mres)
	return bool2int(mres != 0)
}
...
func initGameSession435CC0() error {
	...
	// Reset UI state flags that are NOT cleared between sessions but affect overlay visibility:
	//
	// 1. flag_815132: set to 1 by nox_xxx_wndLoadMainBG_4A2210 (main menu shell) and
	//    nox_xxx_cliWaitForJoinData_43BFE0 (wait-for-join screen). Normally cleared by
	//    nox_client_guiXxxDestroy_4A24A0, but that function is not always called before
	//    CONNECT_RESULT_OK, so it may still be 1 when the second session starts.
	nox_client_gui_flag_815132 = 0
...
func (s *Server) InitialMapLoad() error {
	slog.Info("InitialMapLoad: entered")
	legacy.Sub_4E79B0(0)
	curMap := s.nox_server_currentMapGetFilename_409B30()
	slog.Info("InitialMapLoad: current map", "map", curMap)
	if curMap == "" {
		s.nox_xxx_gameSetMapPath_409D70("tutorial.map")
	}
	s.MapSend.Reset()
	slog.Info("InitialMapLoad: calling nox_xxx_mapExitAndCheckNext_4D1860_server")
	if err := s.nox_xxx_mapExitAndCheckNext_4D1860_server(); err != nil {
		slog.Error("InitialMapLoad: mapExitAndCheckNext failed", "err", err)
		return err
	}
	slog.Info("InitialMapLoad: mapExitAndCheckNext returned ok")
	if debugMainloop {
	...
	slog.Info("InitialMapLoad: done successfully")
	return nil
}

func (s *Server) nox_xxx_servInitialMapLoad_4D17F0() bool {
	slog.Info("servInitialMapLoad: entered")
	if err := s.InitialMapLoad(); err != nil {
		slog.Error("servInitialMapLoad: InitialMapLoad returned error", "err", err)
		gameLog.Println(err)
		return false
	}
	slog.Info("servInitialMapLoad: InitialMapLoad returned ok")
	return true
}
...
func (s *Server) nox_xxx_mapExitAndCheckNext_4D1860_server() error {
	slog.Info("mapExitAndCheckNext: entered")
	if noxflags.HasGame(noxflags.GameClient) {
		noxClient.Nox_client_setCursorType(gui.CursorBusy)
	}
	slog.Info("mapExitAndCheckNext: Sub_4D22B0")
	legacy.Sub_4D22B0()
	slog.Info("mapExitAndCheckNext: netMsgFadeBegin")
	s.Nox_xxx_netMsgFadeBegin_4D9800(false, true)
	slog.Info("mapExitAndCheckNext: scriptOnEvent")
	s.scriptOnEvent(script.EventMapExit)
	v2 := s.getServerMap()
	slog.Info("mapExitAndCheckNext: Sub_500510", "v2", v2)
	legacy.Sub_500510(v2)
	slog.Info("mapExitAndCheckNext: calling mapSwitchLevel")
	s.nox_xxx_mapSwitchLevel_4D12E0(true)
	slog.Info("mapExitAndCheckNext: mapSwitchLevel returned")
	...
	if nox_xxx_gameIsSwitchToSolo_4DB240() {
		...
		slog.Info("mapExitAndCheckNext: switch to solo", "map", v5)
		merr = s.nox_server_loadMapFile_4CF5F0(v5, false)
	} else {
		slog.Info("mapExitAndCheckNext: normal load path")
		...
		slog.Info("mapExitAndCheckNext: calling loadMapFile", "map", v10)
		merr = s.nox_server_loadMapFile_4CF5F0(v10, false)
		slog.Info("mapExitAndCheckNext: loadMapFile returned", "merr", merr)
...
func sub_473840() {
	...
	legacy.Set_nox_wnd_quitMenu_825760(nil)
	legacy.Set_dword_5d4594_1098628(0)
	legacy.Set_dword_5d4594_1098624(0)
	legacy.Set_dword_5d4594_1123520(0)
