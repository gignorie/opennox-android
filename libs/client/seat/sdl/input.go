...
// touchEventHandler is an optional handler for touch events.
// Set by Android-specific code to enable virtual controls.
var (
...
func (win *Window) InputTick() {
	...
	} else if win.pendingTapUp > 0 {
		win.pendingTapUp--
		if win.pendingTapUp == 0 {
			win.inputEvent(&seat.MouseButtonEvent{
				Button:  seat.MouseButtonLeft,
				Pressed: false,
			})
	...
	for {
		...
		case sdl.MouseButtonEvent:
			if uint32(ev.Which) == uint32(sdl.TOUCH_MOUSEID) && filterTouchMouse != nil && filterTouchMouse() {
				continue
			}
		...
		case sdl.TouchFingerEvent:
			if touchEventHandler != nil {
				touchEventHandler(win, &ev)
			}
...
func (win *Window) sendMouseMotion(x, y int32) {
...
func (win *Window) sendMouseButton(button seat.MouseButton, pressed bool) {
	win.inputEvent(&seat.MouseButtonEvent{
		Button:  button,
		Pressed: pressed,
	})
...
func (win *Window) triggerTapClick(x, y int32) {
	win.sendMouseMotion(x, y)
	win.pendingTapDown = 1
}

func (win *Window) pushKey(scancode sdl.Scancode, pressed bool) {
	if int(scancode) < len(scanCodeToKeyNum) {
		key := scanCodeToKeyNum[scancode]
