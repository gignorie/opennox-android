...
func (s *Server) newPlayer(ind ntype.PlayerInd, opts *PlayerOpts) int {
	slog.Info("newPlayer: entered", "ind", ind)
	v2 := opts.Byte152
	opts.Byte152 &= 0x7F
	v3 := v2 >> 7
	...
	slog.Info("newPlayer: creating unit", "ptyp", ptyp)
	punit := s.NewObjectByTypeID(ptyp)
	slog.Info("newPlayer: punit created", "nil", punit == nil)
	if punit == nil {
		slog.Error("newPlayer: punit is nil!")
		return 0
	}
	if ind != server.HostPlayerIndex {
