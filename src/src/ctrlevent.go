...
func (c *CtrlEventHandler) nox_xxx_clientControl_42D6B0_B() {
	...
	for i := indStart; i < indEnd; i++ {
		...
		case player.CCServerOptions:
			...
			ce.Active = false
		case player.CCLaugh:
			nox_xxx_netClientSendSocial(server.HostPlayerIndex, 1, 0, 1)
		case player.CCTaunt:
			nox_xxx_netClientSendSocial(server.HostPlayerIndex, 2, 0, 1)
		case player.CCPoint:
			nox_xxx_netClientSendSocial(server.HostPlayerIndex, 4, 0, 1)
...
func (c *CtrlEventHandler) ensureDefaultBindings() {
	if c == nil {
		return
	}
	// 1. Ensure KeyLeftMouse is strictly bound to EventAction and not corrupted by other events (e.g. SpellSet1)
	hasAction := false
	for it := c.bindings; it != nil; it = it.next {
		for i := 0; i < len(it.keys); i++ {
			if it.keys[i] == keybind.KeyLeftMouse {
				isAction := false
				for _, ev := range it.events {
					if ev == keybind.EventAction {
						isAction = true
						hasAction = true
						break
					}
	...
	if !hasAction {
		c.addBinding(&CtrlEventBinding{
			keys:   []keybind.Key{keybind.KeyLeftMouse},
			events: []keybind.Event{keybind.EventAction},
		})
	}

	// 2. Ensure KeyRightMouse is bound to EventMoveForward
	hasMove := false
	for it := c.bindings; it != nil; it = it.next {
		for _, k := range it.keys {
			if k == keybind.KeyRightMouse {
				for _, ev := range it.events {
					if ev == keybind.EventMoveForward {
						hasMove = true
						break
					}
				}
	...
	if !hasMove {
		c.addBinding(&CtrlEventBinding{
			keys:   []keybind.Key{keybind.KeyRightMouse},
			events: []keybind.Event{keybind.EventMoveForward},
		})
	}

	// 3. Ensure numeric keys 1-5 also invoke quick spell slots 1-5 (for Android overlay)
	slots := []struct {
		key keybind.Key
		ev  keybind.Event
	}{
		{keybind.Key1, keybind.EventInvokeSlot1},
		{keybind.Key2, keybind.EventInvokeSlot2},
		{keybind.Key3, keybind.EventInvokeSlot3},
		{keybind.Key4, keybind.EventInvokeSlot4},
		{keybind.Key5, keybind.EventInvokeSlot5},
	}
	for _, s := range slots {
		if !c.hasDefBinding(s.ev, s.key) {
			c.addBinding(&CtrlEventBinding{
				keys:   []keybind.Key{s.key},
				events: []keybind.Event{s.ev},
			})
		}
	}
}
