			...
			do {
				if (!(*(uint8_t*)(v4 + 8) & 0x10)) {
					...
					if (!(v5 & 0x100) && *(unsigned short*)(v4 + 4) != *getMemU32Ptr(0x5D4594, 2386856) &&
						...
						v6 = nox_xxx_shopGetItemCost_50E3D0(1, 0, (int)v4);
						if (v6 < v3) {
							v3 = v6;
							v2 = (uint32_t*)v4;
						}
...
void nox_server_gotoHome(nox_object_t* obj) {
	...
	if (*(uint8_t*)(v2 + 8) & 2) {
		...
		if ((v3 & 0x8000) == 0) {
			...
			if (v6) {
				*((unaligned_float*)v6 + 1) = *getMemFloatPtr(0x587000, 194136 + 8 * v4[94]) * 10.0 + *(unaligned_float*)(v2 + 56);
				*((unaligned_float*)v6 + 2) = *getMemFloatPtr(0x587000, 194140 + 8 * v4[94]) * 10.0 + *(unaligned_float*)(v2 + 60);
			}
...
int nox_script_GetQuestFloat_514C60() {
	...
	v0 = nox_script_pop();
	*(unaligned_float*)&v2 = sub_500770((char*)nox_script_getString_512E40(v0));
	nox_script_push(v2);
	return 0;
}
...
int nox_script_GiveExp_516190() {
	...
	v2 = nox_server_scriptValToObjectPtr_511B60(v1);
	if (v2) {
		nox_xxx_plyrGiveExp_4EF3A0_exp_level(v2, *(unaligned_float*)&v0);
	}
	return 0;
}
