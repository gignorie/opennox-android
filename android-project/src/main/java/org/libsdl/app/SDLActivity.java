    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mLayout.getViewTreeObserver().addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
            ...
            public void onGlobalLayout() {
                ...
                int totalHeight = mLayout.getRootView().getHeight();
                if (totalHeight > 0) {
                    ...
                    if (Build.VERSION.SDK_INT >= 30) {
                        ...
                        int diff = totalHeight - (r.bottom - r.top);
                        visible = diff > totalHeight * 0.20;
                    }
                    if (!visible && mScreenKeyboardShown) {
...
*/
    ...
    public static boolean isScreenKeyboardShown()
    {
        ...
        if (Build.VERSION.SDK_INT >= 30 && mLayout != null) {
            WindowInsets insets = mLayout.getRootWindowInsets();
            if (insets != null) {
    ...
    static class ShowTextInputTask implements Runnable {
        ...
        public void run() {
            ...
            if (mTextEdit.ic instanceof SDLInputConnection) {
                ((SDLInputConnection) mTextEdit.ic).reset();
            }
            ...
            final InputMethodManager imm = (InputMethodManager) SDL.getContext().getSystemService(Context.INPUT_METHOD_SERVICE);
            if (imm != null) {
                imm.restartInput(mTextEdit);
                boolean ok = imm.showSoftInput(mTextEdit, InputMethodManager.SHOW_FORCED);
                if (!ok) {
...
class SDLMain implements Runnable {
    ...
    public void run() {
        ...
        int status = SDLActivity.nativeRunMain(library, function, arguments);

        Log.v("SDL", "Finished main function, exit code: " + status);

        if (status != 0 && SDLActivity.mSingleton != null && !SDLActivity.mSingleton.isFinishing()) {
            SDLActivity.mSingleton.runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    new AlertDialog.Builder(SDLActivity.mSingleton)
                        .setTitle("OpenNox Stopped")
                        .setMessage("OpenNox exited with code " + status + ".\nCheck /sdcard/opennox/opennox.log for details.")
