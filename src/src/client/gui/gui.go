...
func (g *GUI) ProcessKeys(inp *input.Handler) {
	for _, ev := range inp.KeyboardEvents() {
		g.processKeyEvent(inp, ev.Key, ev.Pressed)
	}
}

func (g *GUI) processKeyEvent(inp *input.Handler, key keybind.Key, pressed bool) {
	if g.focused == nil {
		return
	}
	if key == 0 {
	...
	if pressed && inp.GetKeyFlag(key) {
		return
	}
	ok := false
	for win := g.focused; win != nil; win = win.Parent() {
		if EventRespBool(win.Func93(WindowKeyPress{Key: key, Pressed: pressed})) {
			ok = true
			break
		}
	}
