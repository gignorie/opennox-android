...
mainloop:
	...
	MAINLOOP_EXIT:
		slog.Info("MAINLOOP_EXIT reached", "exitPath", exitPath, "continueMenuOrHost", continueMenuOrHost, "mainloopStopError", mainloopStopError)
		if !exitPath {
			if !continueMenuOrHost || mainloopStopError {
				slog.Warn("MAINLOOP_EXIT: calling cleanup and nox_exit(0)")
				cleanup()
				nox_exit(0)
			}
			// repeat
...
func nox_xxx_clientResetSpriteAndGui_4357D0(noSkip bool) bool {
	...
	*memmap.PtrUint32(0x5D4594, 811064) = uint32(bool2int(nox_client_renderGUI_80828))
	...
	slog.Info("clientResetSpriteAndGui: calling chatInit")
	if !nox_xxx_chatInit_48D7D0() {
		return false
	}
	slog.Info("clientResetSpriteAndGui: Sub_4E4EF0")
	legacy.Sub_4E4EF0()
	slog.Info("clientResetSpriteAndGui: Sub_48D740")
	legacy.Sub_48D740()
	slog.Info("clientResetSpriteAndGui: nox_xxx_gameSetCliConnected(false)")
	nox_xxx_gameSetCliConnected(false)
	slog.Info("clientResetSpriteAndGui: Set_nox_gameDisableMapDraw_5d4594_2650672(0)")
	legacy.Set_nox_gameDisableMapDraw_5d4594_2650672(0)
	slog.Info("clientResetSpriteAndGui: Sub_473930")
	legacy.Sub_473930()
	slog.Info("clientResetSpriteAndGui: Sub_48D4B0(0)")
	legacy.Sub_48D4B0(0)
	slog.Info("clientResetSpriteAndGui: done")
	return true
}
...
func CONNECT_OR_HOST() error {
	slog.Info("CONNECT_OR_HOST: entered")
	if debugMainloop {
	...
	slog.Info("CONNECT_OR_HOST: videoGetGameMode", "mode", mode)
	save := clientCurSave()
	slog.Info("CONNECT_OR_HOST: clientCurSave", "nil", save == nil)
	info := legacy.Nox_xxx_getHostInfoPtr_431770()
	slog.Info("CONNECT_OR_HOST: getHostInfoPtr", "nil", info == nil)
	ci := &save.Player
	slog.Info("CONNECT_OR_HOST: player info", "name", ci.Name(), "class", ci.PlayerClass())
	...
	if noxflags.HasGame(noxflags.GameHost) {
		slog.Info("CONNECT_OR_HOST: GameHost branch")
		if err := nox_xxx_replay_4D3860(&popts); err != nil {
			CONNECT_RESULT_FAIL(err)
			return fmt.Errorf("nox_xxx_replay_4D3860: %w", err)
		}
		if !isDedicatedServer {
			slog.Info("CONNECT_OR_HOST: calling newPlayer")
			netCode := noxServer.newPlayer(server.HostPlayerIndex, &popts)
			slog.Info("CONNECT_OR_HOST: newPlayer returned", "netCode", netCode)
			legacy.ClientSetPlayerNetCode(netCode)
		}
		setVersionCode(NOX_CLIENT_VERS_CODE)
		if !isDedicatedServer {
			slog.Info("CONNECT_OR_HOST: calling receiveCli")
			noxClient.nox_netlist_receiveCli_494E90()
			slog.Info("CONNECT_OR_HOST: receiveCli returned")
		}
		gameSetPlayState(2)
	} else {
		slog.Info("CONNECT_OR_HOST: Client branch")
	...
	slog.Info("CONNECT_OR_HOST: calling CONNECT_RESULT_OK")
	if err := CONNECT_RESULT_OK(); err != nil {
		slog.Error("CONNECT_OR_HOST: CONNECT_RESULT_OK failed", "err", err)
		return fmt.Errorf("connect result: %w", err)
	}
	slog.Info("CONNECT_OR_HOST: CONNECT_RESULT_OK returned nil")
	return nil
}
...
func CONNECT_RESULT_OK() error {
	slog.Info("CONNECT_RESULT_OK: entered")
	if debugMainloop {
	...
	// Reset UI state flags that persist across sessions and can hide the touch overlay:
	// - flag_815132: main-menu shell flag (set by wndLoadMainBG / cliWaitForJoinData)
	// - dword_5d4594_1098628: trader/shop open state (sub_479590)
	// These must be cleared here (not just in initGameSession435CC0) because solo mode
	// (GameHost without GameClient) does NOT call initGameSession435CC0.
	nox_client_gui_flag_815132 = 0
	legacy.Set_dword_5d4594_1098628(0)
	legacy.Set_dword_5d4594_1098624(0)
	legacy.Set_dword_5d4594_1123520(0)
	slog.Info("CONNECT_RESULT_OK: calling replayStartReadingOrSaving")
	if err := noxServer.nox_xxx_replayStartReadingOrSaving_4D38D0(); err != nil {
		slog.Error("CONNECT_RESULT_OK: replayStartReadingOrSaving failed", "err", err)
		return fmt.Errorf("nox_xxx_replayStartReadingOrSaving_4D38D0: %w", err)
	}
	if !noxflags.HasGame(noxflags.GameHost) {
		slog.Info("CONNECT_RESULT_OK: not GameHost, clearing UpdateFunc")
		noxServer.SetUpdateFunc(nil)
	} else {
		slog.Info("CONNECT_RESULT_OK: calling servInitialMapLoad")
		if !noxServer.nox_xxx_servInitialMapLoad_4D17F0() {
			slog.Error("CONNECT_RESULT_OK: servInitialMapLoad failed")
			return errors.New("nox_xxx_servInitialMapLoad_4D17F0 exit")
		}
		slog.Info("CONNECT_RESULT_OK: servInitialMapLoad returned ok")
	}
	if !noxflags.HasGame(noxflags.GameClient) {
		slog.Info("CONNECT_RESULT_OK: not GameClient, clearing DrawFunc")
		noxClient.SetDrawFunc(nil)
	} else {
		slog.Info("CONNECT_RESULT_OK: GameClient branch")
		if !noxflags.HasGame(noxflags.GameFlag21) {
			mode := noxClient.videoGetGameMode()
			slog.Info("CONNECT_RESULT_OK: current video mode", "mode", mode)
			if mode.X == 0 || mode.Y == 0 {
				mode.X = noxDefaultWidth
				mode.Y = noxDefaultHeight
				videoUpdateGameMode(mode)
			}
			slog.Info("CONNECT_RESULT_OK: calling gameUpdateVideoMode(false)")
			if err := gameUpdateVideoMode(false); err != nil {
				slog.Error("CONNECT_RESULT_OK: gameUpdateVideoMode failed", "err", err)
				return fmt.Errorf("gameUpdateVideoMode: %w", err)
			}
			slog.Info("CONNECT_RESULT_OK: gameUpdateVideoMode returned ok")
		}
		slog.Info("CONNECT_RESULT_OK: calling initGameSession435CC0")
		if err := initGameSession435CC0(); err != nil {
			slog.Error("CONNECT_RESULT_OK: initGameSession435CC0 failed", "err", err)
			return fmt.Errorf("failed to init game session: %w", err)
		}
		slog.Info("CONNECT_RESULT_OK: initGameSession435CC0 returned ok")
	}
	if noxflags.HasEngine(noxflags.EngineGameLoopMemdump) {
		legacy.Nox_xxx_gameLoopMemDump_413E30()
	}
...
func (c *Client) map_download_finish() int {
	...
	if !noxflags.HasEngine(noxflags.EngineNoRendering) {
		c.Log.Info("fade after map download")
...
func nox_xxx_gameChangeMap_43DEB0() error {
	...
	if noxflags.HasGame(noxflags.GameFlag24) {
		...
		if v3&2 != 0 && v3&4 != 0 {
			...
			if !noxflags.HasEngine(noxflags.EngineNoRendering) {
				c.Log.Info("fade due to map change")
				legacy.Set_nox_gameDisableMapDraw_5d4594_2650672(1)
				c.r.FadeClearScreen(true, color.Black)
			}
