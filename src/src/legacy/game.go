...
func nox_xxx_mapTraceRay_535250(a1 *C.float4, a2 *C.float2, a3 *C.int2, a4 C.char) int {
	p1 := readUnalignedPointf(unsafe.Pointer(&a1.field_0))
	p2 := readUnalignedPointf(unsafe.Pointer(&a1.field_8))
	var outPos types.Pointf
	outGrid := (*image.Point)(unsafe.Pointer(a3))
	hit := GetServer().S().MapTraceRayAt(p1, p2, &outPos, outGrid, server.MapTraceFlags(a4))
	if a2 != nil {
		writeUnalignedPointf(unsafe.Pointer(a2), outPos)
	}
	if hit {
		return 1
	}
...
func nox_xxx_mapTraceObstacles_50B580(from *nox_object_t, a2 *C.float4) int {
	r := readUnalignedRectf(unsafe.Pointer(a2))
...
func sub_517B70(pos *C.float2, fnc unsafe.Pointer, data unsafe.Pointer) {
	...
	p := readUnalignedPointf(unsafe.Pointer(pos))
	GetServer().S().Map.Sub517B70(p, func(it *server.Object) {
...
func sub_518740(a1 *C.float2, a2 uint8) *nox_waypoint_t {
	p := readUnalignedPointf(unsafe.Pointer(a1))
	return (*nox_waypoint_t)(GetServer().S().Sub_518460(p, a2, true).C())
}
...
func nox_xxx_getUnitsInRectAdv_517ED0(rect *C.float4, fnc unsafe.Pointer, data unsafe.Pointer) {
	r := readUnalignedRectf(unsafe.Pointer(rect))
...
func nox_xxx_mapDamageUnitsAround_4E25B0(a1 *C.float, a2, a3 C.float, a4, a5 int, a6, a7 *nox_object_t) {
	pos := readUnalignedPointf(unsafe.Pointer(a1))
...
func sub_497180(r1, r2 *C.float4, outp *C.float2) int {
	rec1 := readUnalignedRectf(unsafe.Pointer(r1))
	rec2 := readUnalignedRectf(unsafe.Pointer(r2))
	p, ok := server.IntersectLines(rec1, rec2)
	if outp != nil {
		writeUnalignedPointf(unsafe.Pointer(outp), p)
	}
	return bool2int(ok)
}
...
func sub_427980(r1, r2 *C.float4) int {
	rec1 := readUnalignedRectf(unsafe.Pointer(r1))
	rec2 := readUnalignedRectf(unsafe.Pointer(r2))
	return bool2int(server.LineTraceXxx(rec1, rec2))
}
