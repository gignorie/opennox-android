...
## Android Port (v1.8.11-android)

- **Android Platform Support**:
  - Full native port for Android using SDL2 and OpenAL Soft (`armeabi-v7a` ARMv7-A).
  - Standalone APK packaging with AAPT2, D8, and apksigner toolchain (`build_apk.sh`).
  - Storage permissions and data loader for `/sdcard/opennox` (supporting Android 7.0 through Android 14+).
- **Touch & Controls**:
  - Full on-screen touch overlay: virtual analog movement joystick, action buttons (Attack, Jump, Cast, Inventory, Book, Esc).
  - Dynamic HUD/Menu switching: automatic overlay visibility toggle between active gameplay and menus.
  - Multi-touch state machine with stale-touch preemption to prevent touch locking or cursor freezing.
  - Window focus loss and menu transition touch resets (`ResetAndroidTouches`).
- **Engine & Native Stability**:
  - Fixed native crash in `nox_xxx_cliPlrInfoLoadFromFile_41A2E0` caused by unrelocated function pointers in struct arrays (`table_55816`) under Go's ARM32 linker.
  - Added direct PC-relative dispatch and runtime constructor initialization for player section loaders.
  - Movie and intro playback handling with audio stream synchronization.
  - Configurable renderer scaling for widescreen and high-density mobile displays.
